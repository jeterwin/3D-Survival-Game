...
public class DayNNight : MonoBehaviour {
    ...
    [SerializeField]
    private float nightFogIntensity = 0.3f;
    ...
    [SerializeField]
    private Gradient moonAmbientColor;
    ...
    private void AdjustSunColor()
    {
        ...
        if(intensity <= 0)
        {
            RenderSettings.fogColor = nightFogColor.Evaluate(nightFogIntensity);
            RenderSettings.ambientSkyColor = moonAmbientColor.Evaluate(nightFogIntensity);
        }
        else
        {
            RenderSettings.fogColor = dayFogColor.Evaluate(intensity);
            RenderSettings.ambientSkyColor = sunColor.Evaluate(intensity);
        }
    }

    public void AddModule(DNModuleBase module)
    {
        module.EnableShadows();
        moduleList.Add(module);
    }

    public void RemoveModule(DNModuleBase module)
    {
        module.DisableShadows();
